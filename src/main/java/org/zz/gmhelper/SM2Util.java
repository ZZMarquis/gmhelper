...
public class SM2Util extends GMBaseUtil {
  ...
  private static final java.security.spec.ECPoint JDK_G_POINT =
      new java.security.spec.ECPoint(
          SM2Util.G_POINT.getAffineXCoord().toBigInteger(),
          SM2Util.G_POINT.getAffineYCoord().toBigInteger());
  static final java.security.spec.ECParameterSpec JDK_EC_SPEC =
      new java.security.spec.ECParameterSpec(
          SM2Util.JDK_CURVE, SM2Util.JDK_G_POINT, SM2Util.SM2_ECC_N, SM2Util.SM2_ECC_H.intValue());
  ...
  public static AsymmetricCipherKeyPair generateKeyPair() {
    ...
    return BCECUtil.generateKeyPair(SM2Util.DOMAIN_PARAMS, random);
  }
  ...
  public static KeyPair generateBCECKeyPair()
    ...
    ECParameterSpec parameterSpec =
        new ECParameterSpec(SM2Util.CURVE, SM2Util.G_POINT, SM2Util.SM2_ECC_N, SM2Util.SM2_ECC_H);
  ...
  private static BCECPublicKey convertPublicKey(byte[] x509Bytes)
      throws NoSuchProviderException, NoSuchAlgorithmException, InvalidKeySpecException {
    return BCECUtil.convertX509ToECPublicKey(x509Bytes);
  }

  public static BCECPublicKey convertPublicKey(SubjectPublicKeyInfo subPubInfo)
      throws NoSuchProviderException, NoSuchAlgorithmException, InvalidKeySpecException,
          IOException {
    return SM2Util.convertPublicKey(subPubInfo.toASN1Primitive().getEncoded(ASN1Encoding.DER));
  }

  public static byte[] encrypt(BCECPublicKey pubKey, byte[] srcData)
      throws InvalidCipherTextException {
    ECPublicKeyParameters pubKeyParameters = SM2Util.convertPublicKey(pubKey);
    return SM2Util.encrypt(pubKeyParameters, srcData);
  }
  ...
  public static byte[] decrypt(BCECPrivateKey priKey, byte[] sm2Cipher)
      throws InvalidCipherTextException {
    ECPrivateKeyParameters priKeyParameters = SM2Util.convertPrivateKey(priKey);
    return SM2Util.decrypt(priKeyParameters, sm2Cipher);
  }
  ...
  public static SM2Cipher parseSM2Cipher(byte[] cipherText) {
    int curveLength = BCECUtil.getCurveLength(SM2Util.DOMAIN_PARAMS);
    return SM2Util.parseSM2Cipher(curveLength, SM2Util.SM3_DIGEST_LENGTH, cipherText);
  }
   ...
   * @return
   */
  private static SM2Cipher parseSM2Cipher(int curveLength, int digestLength, byte[] cipherText) {
  ...
  public static byte[] encodeSM2CipherToDER(byte[] cipher) throws IOException {
    int curveLength = BCECUtil.getCurveLength(SM2Util.DOMAIN_PARAMS);
    return SM2Util.encodeSM2CipherToDER(curveLength, SM2Util.SM3_DIGEST_LENGTH, cipher);
  }
   ...
   * @throws IOException
   */
  private static byte[] encodeSM2CipherToDER(int curveLength, int digestLength, byte[] cipher)
      throws IOException {
    int startPos = 1;
  ...
  public static byte[] decodeDERSM2Sign(byte[] derSign) {
    ...
    rBytes = SM2Util.fixTo32Bytes(rBytes);
    sBytes = SM2Util.fixTo32Bytes(sBytes);
